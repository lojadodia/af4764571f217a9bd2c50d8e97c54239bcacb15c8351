...
const Banner: React.FC<BannerProps> = ({ className }) => {
  ...
  //min-h-140
  return (
    <div className={cn(`relative ${settings?.env?.HEADER_THEME == "dark" ? " bg-black " : " "} `, className)}>
        ...
        <div>
          <div className="py-5 hidden lg:block"></div>
            ...
            <div className={`${promotion_slider == 'none' ? ' items-center justify-center text-center ' : 'text-left' } hidden lg:block p-5 mt-8  inset-0 w-full flex flex-col  relative`}>
              <h1 className={`${promotion_slider == 'none' ? ' xl:text-5xl' : 'xl:text-4xl' } text-4xl  tracking-tight ${settings?.env?.HEADER_THEME == "dark" ? " text-white " : " text-heading "} dark:text-white font-bold mb-4 xl:mb-5`}>
                {settings?.site?.title}
              </h1>
              <p className={`text-base xl:text-lg ${settings?.env?.HEADER_THEME == "dark" ? " text-white " : " text-heading "} dark:text-white  mb-5 xl:mb-5`}>
